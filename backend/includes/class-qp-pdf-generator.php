...
class QP_PDF_Generator {
    ...
    private function init_pdf($settings) {
        ...
        // Set default font (will use built-in Helvetica for now)
        // Note: For proper Bangla rendering, you'd need to add TTF font support to FPDF
    ...
    private function save_pdf($filename) {
        ...
        $file_path = QP_PDF_UPLOAD_DIR . $filename;
        ...
        return array(
            'path' => $file_path,
            'url'  => QP_PDF_UPLOAD_URL . $filename,
            'filename' => $filename
        );
    }
}
